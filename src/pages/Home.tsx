...
const Home = () => {
  ...
  const handleRowClick = (crypto: CryptoData) => {
    ...
    const contractAddress = crypto.id.split("_")[1]; // Extracting contract address
    if (!contractAddress) {
      console.warn("Invalid contract address:", crypto);
      return;
    }
  
    const url = `https://dexscreener.com/search?q=${contractAddress}`;
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full bg-gray-900 rounded-lg overflow-hidden">
                  ...
                  {cryptoData.map((crypto) => (
                      <tr key={crypto.id} className="hover:bg-gray-800 transition-colors cursor-pointer" onClick={() => handleRowClick(crypto)}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-white">${formatPrice(crypto?.price)}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-white">{formatMarketCap(crypto?.marketCap ?? 0)}</td>
                      </tr>
                  ))}
                  </tbody>
                </table>
